...
type ShaderInfo = ([Temp], [Attribute], [Uniform], [Sampler])

shaderInfo :: [Temp] -> Array Int Temp -> ShaderInfo
shaderInfo sourceTemps allTemps = totalInfo where
	(tempIndices, attributes, uniforms, samplers) = foldr mergeNodeInfo emptyNodeInfo $ map (\Temp { tempNode = node } -> nodeInfo node) sourceTemps
	temps = map (allTemps !) $ unique tempIndices
	totalInfo = (temps, unique attributes, unique uniforms, unique samplers)
...
programInfo State
	...
	vsInfo = shaderInfo vertexTemps allTemps
	psInfo = shaderInfo pixelTemps allTemps

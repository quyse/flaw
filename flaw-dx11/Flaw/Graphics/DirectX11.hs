...
instance Device Dx11Device where
  ...
  createStaticTexture device@Dx11Device
    ...
    let
      dxgiFormat = getDXGIFormat format
      realCount = if count > 0 then count else 1
      TextureMetrics
    ...
    (createResource, srvDesc) <- do
      ...
      let
        subresourceDesc bytesPtr imageOffset TextureMipMetrics
        ...
        subresourceDescs bytesPtr = concatMap (\imageIndex -> map (subresourceDesc bytesPtr $ imageIndex * imageSize) mipsMetrics) [0..(mips - 1)]

      -- switch by texture type
      -- if it's 3D texture
      if depth > 0 then do
        -- arrays of 3D textures not supported
        when (count > 0) $ throwIO $ DescribeFirstException "array of 3D textures is not supported"
        let
          -- texture desc
          ...
          createResource bytesPtr = with desc $ \descPtr -> withArray (subresourceDescs bytesPtr) $ \subresourceDescsPtr ->
          ...
          -- SRV desc
          srvDesc = D3D11_SHADER_RESOURCE_VIEW_DESC_Texture3D
  ...
  createBlendState Dx11Device
    ...
    let
      convertColorSource colorSource = case colorSource of
      ...
      convertAlphaSource alphaSource = case alphaSource of
      ...
      convertOperation operation = case operation of
  ...
  createDepthStencilTarget Dx11Device
    ...
    -- DSV desc
    let
      dsvDesc = D3D11_DEPTH_STENCIL_VIEW_DESC_Texture2D
  ...
  createReadableDepthStencilTarget device@Dx11Device
    ...
    -- SRV desc
    let
      srvDesc = D3D11_SHADER_RESOURCE_VIEW_DESC_Texture2D
  ...
  createStaticVertexBuffer Dx11Device
    ...
    let
      ...
      -- data
      subresourceData bytesPtr = D3D11_SUBRESOURCE_DATA
  ...
  createProgram Dx11Device
    ...
    let
      createInputLayout attributes vertexShaderByteCode = allocateCOMObject $ B.unsafeUseAsCStringLen vertexShaderByteCode $ \(byteCodePtr, byteCodeSize) -> let
    ...
    let
      compileShader HlslShader
        ...
        let
          optimize = not debug
          flags = fromIntegral $ fromEnum D3DCOMPILE_ENABLE_STRICTNESS .|. fromEnum D3DCOMPILE_PACK_MATRIX_COLUMN_MAJOR
          ...
          -- get cache key
          cacheKey = S.encode (source, entryPoint, profile, flags)
        -- try to use cache
    ...
    let
      ...
      createVertexShader bytecode = allocateCOMObject $ B.unsafeUseAsCStringLen bytecode $ \(ptr, len) ->
      ...
      createPixelShader bytecode = allocateCOMObject $ B.unsafeUseAsCStringLen bytecode $ \(ptr, len) ->
...
dx11GetDepthStencilState Dx11Context
  ...
  case HM.lookup code depthStencilStateCache of
    ...
    Nothing -> do
      let
        stencilOpDesc = D3D11_DEPTH_STENCILOP_DESC
...
dx11UpdateContext context@Dx11Context
  ...
  -- framebuffer
  refSetup actualFrameBufferRef desiredFrameBufferRef $ \desiredFrameBuffer -> let
    Dx11FrameBufferId renderTargets depthStencilTarget = desiredFrameBuffer
    renderTargetsInterfaces = [pokeCOMObject rtv | Dx11RenderTargetId rtv <- renderTargets]
    depthStencilInterface = case depthStencilTarget of
      Dx11DepthStencilTargetId interface -> pokeCOMObject interface
      Dx11NullDepthStencilTargetId -> nullPtr
    in withArray renderTargetsInterfaces $ \renderTargetsInterfacesPtr ->
  ...
  -- viewport
  refSetup actualViewportRef desiredViewportRef $ \(Vec4 left top right bottom) -> let
    -- set new viewport
  ...
  vectorSetup actualVertexBuffersVector desiredVertexBuffersVector $ \desiredVertexBuffers -> do
    ...
    allocaArray buffersCount $ \buffersInterfacesPtr -> allocaArray buffersCount $ \stridesPtr -> allocaArray buffersCount $ \offsetsPtr -> do
      flip V.imapM_ desiredVertexBuffers $ \i vertexBuffer -> do
        let
          (bufferInterfacePtr, stride) = case vertexBuffer of
  ...
  refSetup actualIndexBufferRef desiredIndexBufferRef $ \desiredIndexBuffer -> do
    let
      (indexBufferInterfacePtr, format, primitiveTopology) = case desiredIndexBuffer of
    ...
    allocaArray samplersCount $ \srvInterfacesPtr ->
      allocaArray samplersCount $ \ssInterfacesPtr -> do
        flip V.imapM_ desiredSamplers $ \i (texture, samplerState) -> do
          let
            (srvInterfacePtr, ssInterfacePtr) = case texture of
  ...
  refSetup actualBlendStateRef desiredBlendStateRef $ \desiredBlendState -> do
    let
      blendStateInterfacePtr = case desiredBlendState of
  ...
  do
    ...
    when (actualDepthTestFunc /= desiredDepthTestFunc || actualDepthWrite /= desiredDepthWrite) $ do
      let
        comparisonFunc = dx11DepthTestFunc desiredDepthTestFunc
        code = dx11CalcDepthStencilStateCode comparisonFunc desiredDepthWrite
